...
class Vocabulary:
    ...
    def fit(self, numClusters, samplePercent, randomState= None):
        #open the database and grab all the total number of features
        db = h5py.File(self.dbPath)
        totalFeatures = db["features"].shape[0]

        # determine the number of features to sample, generate the indexes of the sample, sorting them in ascending order to speedup
        # access time from the HDF5 database
        ...
        idxs.sort()
        data=[]
        self._debug("start sampling ..")
        # loop over the randomly sampled indexes and accumalate the features to cluster
        for i in idxs:
