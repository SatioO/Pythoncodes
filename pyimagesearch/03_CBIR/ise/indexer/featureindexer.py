...
class FeatureIndexer(BaseIndexer):
    def __init__(self, dbPath, estNumImages = 500, maxBufferSize = 50000, dbResizeFactor = 2, verbose = True):
        ...
        super().__init__(dbPath, estNumImages = estNumImages, maxBufferSize = maxBufferSize,
        ...
        #initialize the ImagesID buffer , index buffer and the keypoints + features buffer
        self.imageIDBuffer = []
        self.indexBuffer = []
    ...
    def _createDatasets(self):
        # compute the avg number of features extracted from the initial buffer and use this number to determine the approximate number of features for the entire dataset
        ...
        self.imageIDDB = self.db.create_dataset("image_ids", (self.estNumImages,), maxshape=(None,), dtype = h5py.special_dtype(vlen=str))
    ...
    def _writeBuffers(self):
        ...
        # reset the buffers and feature count
        self.imageIDBuffer = []
        self.indexBuffer = []
        self.featuresBuffer = None
        self.totalFeatures = 0
