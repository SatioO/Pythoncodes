...
class BaseIndexer(object):
    ...
    def _writeBuffer(self, dataset, datasetName, buf, idxName, sparse= False):
        ...
        # otherwise, assume that the buffer is a Numpy/Scipy array, so compute the ending index based on the array shape
        else:
    ...
    def _resizeDataset(self, dataset, dbName, baseSize=0, finished=0):
        ...
        self._debug("old size of `{}`: {:,}; newSize: {:,}".format(dbName,origSize, newSize))


    def _debug(self, msg, msgType="[INFO]"):
        #check to see if the msg should be printed
        if self.verbose:
